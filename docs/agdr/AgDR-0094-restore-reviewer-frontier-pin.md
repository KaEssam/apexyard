...
- Discovery gap findings from the same xhigh review — the `.sql`-only extension gap for two SQL-scoped csharp handbooks, and `handbooks/README.md`'s stale "fifth bucket" count — were fixed in the same pass as this AgDR. The third finding (`handbooks/language/dotnet/` never wired into Rex's discovery script) was superseded before this AgDR landed: PR #6 (KaEssam/apexyard, 2026-07-27) relocated the two dotnet-bucket handbooks into `handbooks/language/csharp/` instead, on Rex's own review suggestion, since `dotnet/` was never a documented discovery bucket. No separate wiring is needed as a result — the `.{cs,sql}` trigger fix in this AgDR already covers them.
- `test_agent_wrap_shape.sh` was briefly (same-day) modified to *accept* `model: inherit` on Rex/Hakim as non-regression, in the process of fixing an unrelated red-CI blocker before this AgDR's existence was discovered. That test change is reverted alongside this AgDR landing, restoring the original "utility agents must pin an explicit model" invariant.
...
- Landed via: `fix/GH-1-restore-reviewer-frontier-pin` (2026-07-27), reconciled against PR #6's dotnet→csharp handbook relocation
